...
def asynchronous():
    ...
    coin_start=0
    while True:
        cursor=users_table.select().with_only_columns([users_table.c.symbol,users_table.c.full_name]).where((users_table.c.rank_coin>=coin_start)&(users_table.c.rank_coin<=coin_start+5))
...
def asynchronous_date():
    ...
    cursor=users_table.select().with_only_columns([users_table.c.symbol,users_table.c.full_name]).where((users_table.c.rank_coin>=0)&(users_table.c.rank_coin<10))
    db = con.execute(cursor)
